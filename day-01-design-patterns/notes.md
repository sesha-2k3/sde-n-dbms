...
- Interface Segregation Principle:
    - 
    - Example: Here, since the validation for different payment methods differ, we can use a separate authorizer that takes care of only the authorization part. This is not directly related to the payment processor, but it is a good example of how to use the Interface Segregation Principle. This way, we can also avoid violating the Liskov Substitution Principle.
    ```python
    """
    Before refactoring (violates Interface Segregation Principle).
    """
    from abc import ABC, abstractmethod
    ...
    """
    After refactoring (satisfies Interface Segregation Principle).
    """
    from abc import ABC, abstractmethod
...
- Dependency Inversion Principle:
    - This principle states that a concrete class should not depend on another concrete class. Instead, let us depend on a high level abstraction, say an interface, and we should have a concrete class implement that interface. The advantages of this is, that we can change the implementation of the concrete class without changing the high level abstraction, and hence without changing the other parts of the application. 
    - Example:
    ```python
    """
    Before refactoring (violates Dependency Inversion Principle).
    """
    from abc import ABC, abstractmethod
    ...
    """
    After refactoring (satisfies Dependency Inversion Principle).
    """
    from abc import ABC, abstractmethod
